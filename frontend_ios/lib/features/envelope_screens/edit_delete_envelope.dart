...
class EditDeleteEnvelopeScreen extends StatefulWidget {
  const EditDeleteEnvelopeScreen({super.key, required this.envelope});

  final Envelope envelope;

  @override
  State<EditDeleteEnvelopeScreen> createState() => _EditDeleteEnvelopeScreenState();
}

class _EditDeleteEnvelopeScreenState extends State<EditDeleteEnvelopeScreen> {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      setState(() {
        ...
        _errorMessage = cleaned.isEmpty ? 'Unable to update envelope.' : cleaned;
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _handleDelete() async {
    ...
    try {
      await _apiService.deleteEnvelope(widget.envelope.id);
      if (!mounted) return;
      Navigator.of(context).pop({'status': 'deleted'});
    } catch (e) {
      setState(() {
        final cleaned = e.toString().replaceFirst(RegExp(r'^Exception: ?'), '');
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  _SaveButton(onPressed: _handleSave, isSaving: _isSaving || _isDeleting),
                ],
              ),
            ),
            Expanded(
